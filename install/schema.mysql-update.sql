...
-- Add column profile stats to indicator_dsd
ALTER TABLE `indicator_dsd`
  ADD COLUMN `sum_stats` json DEFAULT NULL
  COMMENT 'Column profile from DuckDB: row_count, non_null_count, null_count, distinct_count, freq (max 100); missing = NULL/trim empty'
  AFTER `metadata`;

-- Local codelists (project-scoped vocabularies)
...
-- Add global/local codelist reference columns to indicator_dsd (run after local_codelists exists)
ALTER TABLE `indicator_dsd`
